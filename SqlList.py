...
P_In_sql = "INSERT INTO Products (name,num,price) values(?,?,?)"
P_Del_sql = "DELETE from Products where id = "
P_Ch_sql = "Update Products set name = ?, num = ?, price = ? where id = ?"

def Create():
    with con:
        con.execute(
            """CREATE TABLE IF NOT EXISTS Products  (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, name TEXT, num INTEGER, price INTEGER);""")
        con.execute(
